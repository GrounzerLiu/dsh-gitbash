 ...
 *
 * Registers the same `bash` tool contract the shipped
 * `@deepseek-ai/dsh-tool-bash` registers on POSIX — same name, description
 * shape, terminal-card presentation, exit-status markers, background-job
 * support, and sandbox-escalation flow — but consumes `ctx.gitbash` (the
 * Git Bash executor), so it works on Windows, where the shipped bash tool is
 * disabled by design.
 *
...
/** Present completed foreground output as a terminal; background acknowledgements and errors use generic fenced output. */
...
function processOutcome(proc) {
	if (proc.status === "failed") {
		return {
			status: "failed",
			detail: proc.spawnError ?? "spawn failed before the process started"
		};
	}
	...
	ctx.tools.register(
		defineTool({
			...
			async execute(args, exec) {
				...
				if (args.run_in_background === true) {
					...
					return {
						...
						jobId: jobs.start({
							...
							run: () => {
								const proc = ctx.gitbash.start(ctx.gitbash.resolve(request));
								return {
				...
				const result = await ctx.gitbash.run(
					ctx.gitbash.resolve({
						...request,
						signal: exec.signal
					})
